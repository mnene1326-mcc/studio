    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="rounded-3xl border-none p-8 max-w-[90vw] sm:max-w-md">
        <DialogHeader className="items-center text-center space-y-2">
          ...
          <DialogDescription className="text-xs font-bold text-gray-400">Appoint or remove Sellers and Agents.</DialogDescription>
        </DialogHeader>
        <div className="space-y-6 py-6">
          <Input placeholder="Numeric MatchFlow ID" value={targetId} onChange={(e) => setTargetId(e.target.value)} className="rounded-2xl h-14 border-gray-100 bg-gray-50 font-black text-center text-lg" />
        </div>
        <DialogFooter className="flex-col gap-2">
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={() => handleRoleUpdate('isCoinSeller', true)} disabled={loading} className="bg-blue-600 h-12 rounded-full text-[10px] font-black uppercase">Make Seller</Button>
            <Button onClick={() => handleRoleUpdate('isCoinSeller', false)} disabled={loading} variant="outline" className="h-12 rounded-full text-[10px] font-black uppercase">Revoke Seller</Button>
            <Button onClick={() => handleRoleUpdate('isAgent', true)} disabled={loading} className="bg-purple-600 h-12 rounded-full text-[10px] font-black uppercase">Make Agent</Button>
            <Button onClick={() => handleRoleUpdate('isAgent', false)} disabled={loading} variant="outline" className="h-12 rounded-full text-[10px] font-black uppercase">Revoke Agent</Button>
          </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="h-20 bg-pink-500 hover:bg-pink-600 rounded-2xl border-none shadow-xl flex flex-col items-center justify-center gap-1 text-white active:scale-95 transition-all col-span-2 mt-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">Join Agency</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-3xl p-8 max-w-[90vw]">
        <DialogHeader className="items-center text-center">
          <DialogTitle className="text-xl font-black">Enter Agency Code</DialogTitle>
          <DialogDescription className="text-xs">Ask your agent for their 5-digit code.</DialogDescription>
        </DialogHeader>
        <div className="py-6">
          <Input maxLength={5} placeholder="e.g. 54321" value={code} onChange={(e) => setCode(e.target.value)} className="rounded-2xl h-16 text-center text-2xl font-black tracking-[0.5em]" />
        </div>
        <Button onClick={handleJoin} disabled={loading || code.length !== 5} className="w-full h-14 bg-pink-500 rounded-full font-black uppercase tracking-widest">
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button className="h-20 bg-blue-600 hover:bg-blue-700 rounded-2xl border-none shadow-xl flex flex-col items-center justify-center gap-1 text-white active:scale-95 transition-all col-span-2 mt-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">Agency Center</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-3xl p-8 max-w-[90vw]">
        <DialogHeader>
          <DialogTitle className="text-xl font-black">Agency Management</DialogTitle>
        </DialogHeader>
        <div className="py-6 space-y-4">
          {user.agencyId ? (
            <div className="text-center space-y-4">
              <div className="bg-blue-50 p-6 rounded-3xl">
                <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2">Your Agency Code</p>
                <h3 className="text-4xl font-black text-blue-600 tracking-[0.2em]">{user.agencyId}</h3>
              </div>
              <Button asChild className="w-full h-14 bg-blue-600 rounded-full font-black uppercase tracking-widest">
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleCreate} disabled={loading} className="w-full h-14 bg-blue-600 rounded-full font-black uppercase tracking-widest">
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="h-20 bg-gradient-to-br from-yellow-400 to-orange-500 hover:opacity-90 rounded-2xl border-none shadow-xl flex flex-col items-center justify-center gap-1 text-white active:scale-95 transition-all col-span-2 mt-4">
          <div className="flex items-center gap-2"><Trophy className="w-6 h-6" /><span className="text-sm font-black uppercase tracking-widest">Award Coins Tool</span></div>
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-3xl border-none p-8 max-w-[90vw] sm:max-w-md">
        <DialogHeader className="items-center text-center space-y-2">
        ...
        <div className="space-y-6 py-6">
          <Input placeholder="Numeric MatchFlow ID" value={targetId} onChange={(e) => setTargetId(e.target.value)} className="rounded-2xl h-14 border-gray-100 bg-gray-50 font-black text-center text-lg" />
          <Input type="number" placeholder="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="rounded-2xl h-14 border-gray-100 bg-gray-50 font-black text-center text-lg" />
        </div>
        <DialogFooter>
          <Button onClick={handleAward} disabled={loading || !targetId || !amount} className="w-full h-16 rounded-full bg-black text-white font-black uppercase tracking-widest text-sm">
      ...
      <div className="relative z-10">
        <header className="relative pt-12 pb-10 px-6 flex flex-col items-center text-center">
          ...
          <div className="inline-flex items-center gap-1.5 cursor-pointer" onClick={handleCopyId}>
            <p className="text-white/70 font-bold text-[9px] uppercase">ID: {profile.matchFlowId}</p>
        ...
        <main className="px-6 space-y-6">
          <div className="grid grid-cols-2 gap-4 relative z-20 -mt-6">
            <Button className="h-20 bg-white hover:bg-gray-50 rounded-2xl border-none shadow-xl flex flex-col items-center justify-center gap-1 text-[#00A2FF]" onClick={() => router.push("/recharge")}>
              <div className="flex items-center gap-1.5"><CircleDollarSign className="w-5 h-5" /><span className="text-sm font-black">{profile.coins || 0}</span></div>
              <span className="text-[8px] font-black uppercase opacity-60">Recharge Coins</span>
            </Button>
            <Button className="h-20 bg-white hover:bg-gray-50 rounded-2xl border-none shadow-xl flex flex-col items-center justify-center gap-1 text-black" onClick={() => router.push("/income")}>
              <div className="flex items-center gap-1.5"><Gem className="w-5 h-5 text-[#4285F4]" /><span className="text-sm font-black">{profile.diamonds || 0}</span></div>
              <span className="text-[8px] font-black uppercase opacity-60">Diamond Income</span>
            </Button>
            ...
            {profile.agencyStatus === 'pending' && (
              <div className="col-span-2 bg-yellow-50 p-4 rounded-2xl border border-yellow-100 text-center">
                <p className="text-[10px] font-black text-yellow-600 uppercase tracking-widest">Agency Application Pending</p>
              </div>
            )}
             {profile.agencyStatus === 'approved' && profile.agencyId && (
              <div className="col-span-2 bg-green-50 p-4 rounded-2xl border border-green-100 text-center flex items-center justify-center gap-2">
                ...
                <p className="text-[10px] font-black text-green-600 uppercase tracking-widest">Member of Agency: {profile.agencyId}</p>
              </div>
            )}
          </div>
            ...
            <div className="flex flex-col">
              <Button variant="ghost" className="h-16 justify-between px-5 rounded-none" asChild>
                <Link href="/verify-identity">
                  <div className="flex items-center gap-4"><div className="bg-green-50 p-2.5 rounded-xl"><ShieldCheck className="w-5 h-5 text-green-600" /></div><span className="font-black text-xs text-black">Identity Verification</span></div>
                  {(profile.isVerified || profile.isAdmin) && <span className="text-[10px] font-black text-green-500 uppercase">Verified</span>}
                  <ChevronRight className="w-4 h-4 text-gray-300" />
                </Link>
              </Button>
              <Button variant="ghost" className="h-16 justify-between px-5 rounded-none" asChild>
                <Link href="/support"><div className="flex items-center gap-4"><div className="bg-blue-50 p-2.5 rounded-xl"><Headphones className="w-5 h-5 text-blue-600" /></div><span className="font-black text-xs text-black">Support Center</span></div><ChevronRight className="w-4 h-4 text-gray-300" /></Link>
              </Button>
              <Button variant="ghost" className="h-16 justify-between px-5 rounded-none" asChild>
                <Link href="/settings"><div className="flex items-center gap-4"><div className="bg-gray-50 p-2.5 rounded-xl"><Settings className="w-5 h-5 text-gray-600" /></div><span className="font-black text-xs text-black">App Settings</span></div><ChevronRight className="w-4 h-4 text-gray-300" /></Link>
              </Button>
            </div>
          </div>
        </main>
